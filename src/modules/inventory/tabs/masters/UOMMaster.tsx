/**
 * UOM (Unit of Measurement) Master
 * Manage global UOM codes used across modules
 */

'use client';

import React, { useState, useMemo } from 'react';
import {
  Card,
  Table,
  Button,
  Space,
  Input,
  Form,
  Modal,
  Drawer,
  Tag,
  message,
  Divider,
  Upload,
  Typography,
} from 'antd';
import {
  PlusOutlined,
  EditOutlined,
  DeleteOutlined,
  SearchOutlined,
  UploadOutlined,
  DownloadOutlined,
} from '@ant-design/icons';
import { generateSampleExcel } from '../../components/onboarding/utils/sampleExcelGenerator';
import * as XLSX from 'xlsx';

const { Text } = Typography;

interface UOMRecord {
  id: string;
  uomCode: string;
  uomName: string;
  conversionBase?: string;
  status: 'active' | 'inactive';
  createdDate: Date;
}

const SAMPLE_UOMS: UOMRecord[] = [
  {
    id: '1',
    uomCode: 'KG',
    uomName: 'Kilogram',
    conversionBase: '1 KG = 1000 G',
    status: 'active',
    createdDate: new Date('2025-12-15'),
  },
  {
    id: '2',
    uomCode: 'MTR',
    uomName: 'Meter',
    conversionBase: '1 MTR = 100 CM',
    status: 'active',
    createdDate: new Date('2025-12-15'),
  },
  {
    id: '3',
    uomCode: 'PCS',
    uomName: 'Piece',
    status: 'active',
    createdDate: new Date('2025-12-15'),
  },
  {
    id: '4',
    uomCode: 'ROLL',
    uomName: 'Roll',
    status: 'active',
    createdDate: new Date('2025-12-15'),
  },
  {
    id: '5',
    uomCode: 'CONE',
    uomName: 'Cone',
    status: 'active',
    createdDate: new Date('2025-12-15'),
  },
];

const UOMMasterScreen: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<UOMRecord[]>(SAMPLE_UOMS);
  const [drawerVisible, setDrawerVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<UOMRecord | null>(null);
  const [searchText, setSearchText] = useState('');

  const handleFileSelect = async (file: File) => {
    try {
      const arrayBuffer = await file.arrayBuffer();
      const workbook = XLSX.read(arrayBuffer);
      const worksheet = workbook.Sheets[workbook.SheetNames[0]];
      const jsonData: any[] = XLSX.utils.sheet_to_json(worksheet);

      const importedData: UOMRecord[] = jsonData.map((row, index) => ({
        id: `imported-${Date.now()}-${index}`,
        uomCode: row['UOM Code'] || '',
        uomName: row['UOM Name'] || '',
        conversionBase: row['Conversion Base'] || '',
        status: (row['Status']?.toLowerCase() === 'active' ? 'active' : 'inactive') as 'active' | 'inactive',
        createdDate: new Date(),
      }));

      setData([...data, ...importedData]);
      message.success(`${importedData.length} UOM records imported successfully`);
    } catch (error) {
      message.error('Failed to read Excel file');
      console.error(error);
    }
  };

  const handleDownloadSample = () => {
    generateSampleExcel('uom');
    message.success('Sample Excel downloaded');
  };

  const filteredData = useMemo(() => {
    return data.filter((uom) =>
      searchText === '' ||
      uom.uomCode.toLowerCase().includes(searchText.toLowerCase()) ||
      uom.uomName.toLowerCase().includes(searchText.toLowerCase())
    );
  }, [data, searchText]);

  const handleAdd = () => {
    setEditingRecord(null);
    form.resetFields();
    setDrawerVisible(true);
  };

  const handleEdit = (record: UOMRecord) => {
    setEditingRecord(record);
    form.setFieldsValue(record);
    setDrawerVisible(true);
  };

  const handleDelete = (id: string) => {
    Modal.confirm({
      title: 'Delete UOM',
      content: 'Are you sure you want to delete this UOM?',
      okText: 'Delete',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk() {
        setData(data.filter((item) => item.id !== id));
        message.success('UOM deleted successfully');
      },
    });
  };

  const handleSubmit = async (values: any) => {
    if (editingRecord) {
      setData(
        data.map((item) =>
          item.id === editingRecord.id ? { ...item, ...values } : item
        )
      );
      message.success('UOM updated successfully');
    } else {
      const newUOM: UOMRecord = {
        ...values,
        id: Date.now().toString(),
        createdDate: new Date(),
      };
      setData([newUOM, ...data]);
      message.success('UOM created successfully');
    }
    setDrawerVisible(false);
    form.resetFields();
  };

  const columns = [
    {
      title: 'UOM Code',
      dataIndex: 'uomCode',
      key: 'uomCode',
      width: 100,
      render: (text: string) => <strong>{text}</strong>,
    },
    {
      title: 'UOM Name',
      dataIndex: 'uomName',
      key: 'uomName',
      width: 150,
    },
    {
      title: 'Conversion Base',
      dataIndex: 'conversionBase',
      key: 'conversionBase',
      width: 150,
      render: (text: string) => text || '-',
    },
    {
      title: 'Status',
      dataIndex: 'status',
      key: 'status',
      width: 100,
      render: (status: string) => (
        <Tag color={status === 'active' ? 'green' : 'red'}>
          {status.charAt(0).toUpperCase() + status.slice(1)}
        </Tag>
      ),
    },
    {
      title: 'Created Date',
      dataIndex: 'createdDate',
      key: 'createdDate',
      width: 130,
      render: (date: Date) => new Date(date).toLocaleDateString(),
    },
    {
      title: 'Actions',
      key: 'actions',
      width: 100,
      render: (_: any, record: UOMRecord) => (
        <Space>
          <Button
            type="link"
            size="small"
            icon={<EditOutlined />}
            onClick={() => handleEdit(record)}
          />
          <Button
            type="link"
            danger
            size="small"
            icon={<DeleteOutlined />}
            onClick={() => handleDelete(record.id)}
          />
        </Space>
      ),
    },
  ];

  return (
    <div style={{ padding: '16px' }}>
      <Card
        title="UOM (Unit of Measurement) Master"
        extra={
          <Space>
            <Input.Search
              placeholder="Search UOM code or name..."
              prefix={<SearchOutlined />}
              value={searchText}
              onChange={(e) => setSearchText(e.target.value)}
              style={{ width: 250 }}
              allowClear
            />
            <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>
              Add UOM
            </Button>
          </Space>
        }
      >
        <div style={{ marginBottom: 16, padding: '12px 16px', background: 'var(--page-bg)', borderRadius: 8 }}>
          <Space size="middle">
            <Upload
              accept=".xlsx,.xls"
              showUploadList={false}
              beforeUpload={() => false}
              onChange={(info) => {
                const file = info.file.originFileObj || info.file;
                if (file) handleFileSelect(file as File);
              }}
            >
              <Button icon={<UploadOutlined />} size="large" style={{ minWidth: 160 }}>
                Upload Excel
              </Button>
            </Upload>
            <Button icon={<DownloadOutlined />} size="large" onClick={handleDownloadSample} style={{ minWidth: 180 }}>
              Download Sample Excel
            </Button>
          </Space>
          <div style={{ marginTop: 8 }}>
            <Text type="secondary" style={{ fontSize: 12 }}>
              Download sample to understand the required format. Replace sample data with your actual factory data.
            </Text>
          </div>
        </div>
        <Divider style={{ margin: '16px 0' }} />
        <Table
          columns={columns}
          dataSource={filteredData}
          rowKey="id"
          pagination={{ pageSize: 15, showSizeChanger: true }}
          size="small"
        />
      </Card>

      {/* UOM Form Drawer */}
      <Drawer
        title={editingRecord ? 'Edit UOM' : 'Add New UOM'}
        placement="right"
        className="inventory-drawer"
        onClose={() => {
          setDrawerVisible(false);
          form.resetFields();
        }}
        open={drawerVisible}
        width={500}
        footer={
          <Space>
            <Button onClick={() => setDrawerVisible(false)}>Cancel</Button>
            <Button type="primary" onClick={() => form.submit()}>
              {editingRecord ? 'Update' : 'Create'}
            </Button>
          </Space>
        }
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item
            name="uomCode"
            label="UOM Code"
            rules={[{ required: true, message: 'Please enter UOM code' }]}
          >
            <Input placeholder="e.g., KG, MTR, PCS" maxLength={10} />
          </Form.Item>

          <Form.Item
            name="uomName"
            label="UOM Name"
            rules={[{ required: true, message: 'Please enter UOM name' }]}
          >
            <Input placeholder="e.g., Kilogram, Meter, Piece" />
          </Form.Item>

          <Form.Item name="conversionBase" label="Conversion Base">
            <Input placeholder="e.g., 1 KG = 1000 G" />
          </Form.Item>

          <Form.Item
            name="status"
            label="Status"
            initialValue="active"
            rules={[{ required: true, message: 'Please select status' }]}
          >
            <select style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid var(--color-d9d9d9)' }}>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
          </Form.Item>

        </Form>
      </Drawer>
    </div>
  );
};

export default UOMMasterScreen;
